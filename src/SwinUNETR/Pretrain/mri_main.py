...
from utils.mri_data_utils import get_loader
...
def main():
    ...
    if args.distributed:
        if dist.get_rank() == 0:
            torch.save(model.state_dict(), logdir + "mri_ssl_final_model.pth")
        dist.destroy_process_group()
    else:
        torch.save(model.state_dict(), logdir + "mri_ssl_final_model.pth")

    save_ckp(checkpoint, logdir + "/mri_ssl_model_final_epoch.pt")
