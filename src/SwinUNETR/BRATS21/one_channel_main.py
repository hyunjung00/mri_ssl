...
from utils.one_channel_data_utils import get_loader
...
parser = argparse.ArgumentParser(
    description="Swin UNETR segmentation pipeline for BRATS Challenge"
)
parser.add_argument(
...
parser.add_argument(
    "--data_dir", default="../../../data/brats2021", type=str, help="dataset directory"
)
parser.add_argument(
    "--json_list",
...
def main_worker(gpu, args):
    ...
    if args.pretrained_dir == "./pretrained_models/ssl_ct_pretrained/":
        ...
        print("######################################################")
        print("Using CT pretrained self-supervied Swin UNETR backbone weights !")

    if (
        args.pretrained_dir
        == "./pretrained_models/ssl_mri_pretrained/ssl_brats_pretrained/"
    ):
        model_dict = torch.load(pretrained_pth)
        state_dict = model_dict["state_dict"]
        # fix potential differences in state dict keys from pre-training to
        ...
        # the decoder weights untouched (CNN UNet decoder).
        model.load_state_dict(state_dict, strict=False)
        print("Using MRI pretrained SSL (BRATS) Swin UNETR backbone weights!")
